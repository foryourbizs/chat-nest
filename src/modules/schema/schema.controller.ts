...
export class SchemaController {
  ...
  async index() {
    try {
      ...
      const modelsInfo: SchemaEntityInfo[] = metadata.map((entityMetadata) => {
        // 컬럼 정보 추출
        const columns: ColumnMetadata[] = entityMetadata.columns.map(
          (column) => ({
            ...
            generationStrategy: column.generationStrategy as
              | 'uuid'
              | 'increment'
              | 'rowid'
              | 'identity'
        ...
        // 관계 정보 추출
        const relations: RelationInfo[] = entityMetadata.relations.map(
          (relation) => ({
            name: relation.propertyName,
            type: relation.relationType as
              | 'one-to-one'
              | 'one-to-many'
              | 'many-to-one'
              | 'many-to-many',
        ...
        // 기본 키 정보
        const primaryKeys: PrimaryKeyInfo[] = entityMetadata.primaryColumns.map(
          (column) => ({
            name: column.propertyName,
            databaseName: column.databaseName,
            type: column.type as string,
        ...
        // 외래키
        const foreignKeys: ForeignKeyInfo[] = entityMetadata.foreignKeys.map(
          (fk) => ({
            name: fk.name,
            columns: fk.columns.map((column) => column.databaseName),
            referencedTable: fk.referencedTablePath,
            referencedColumns: fk.referencedColumns.map(
  ...
  async show(@Param('entityName') entityName: string) {
    try {
      const metadata = this.dataSource.entityMetadatas;
      const entityMetadata = metadata.find(
        (meta) =>
